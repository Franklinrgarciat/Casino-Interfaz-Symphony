...
namespace CapaDatos
{
    public class DatosCasino
    {
        ...
        public int TraerTalonario(string tipo, string tComp, string ptoVenta)
        {
            SqlConnection cn = new SqlConnection(ConfigurationManager.AppSettings.Get("Conexion").ToString());
            SqlCommand cm = new SqlCommand();
            int Talonario = 0;
            try
            {
                using (cn)
                {
                    cn.Open();
                    cm.Connection = cn;
                    cm.CommandText = "SELECT TALONARIO FROM GVA43 WHERE TIPO='" + tipo + "'AND COMPROB='" + tComp + "' AND SUCURSAL='" + ptoVenta + "'";
                    cm.ExecuteNonQuery();
                    SqlDataReader dr = cm.ExecuteReader();
                    if (dr.Read())
                    {
        ...
        public SqlCommand InsertarAsientoComprobante()
        {
            try
            {
                ...
                cm.CommandText = "insert into  asiento_comprobante_gv " +
                    ...
                    " from gva12 " +
                    " where t_comp<>'REC' and ncomp_in_v not in (select ncomp_in_v from asiento_comprobante_gv)";
                return cm;
            }
            catch (Exception ex)
            {
